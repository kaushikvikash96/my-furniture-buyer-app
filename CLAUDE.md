...
**One external service beyond the shop's own API:** the "Ask" assistant
(`lib/assistant.ts`) calls an Azure OpenAI `gpt-5-mini` deployment (env vars
`AZURE_OPENAI_*` in `.env`), also via plain `fetch` — no SDK, same rule as
above. It gets three read-only tools over the real catalogue/balance and one
`recommend_product` tool that only stages a suggestion; it can never call
`placeRealOrder` itself. See [architecture.md §7](architecture.md) for why
that boundary exists and `lib/assistant.ts`'s file comment for the tool loop.
...
**Ask assistant added (2026-07-29), not one of requirements 1–3.** A text box
at `/assistant` (`app/assistant/page.tsx`) lets a buyer type a plain-English
request; `lib/assistant.ts` runs a tool-use loop against Azure OpenAI with
three read-only tools over the real catalogue/balance, applying its own
judgment for anything the shop's search can't do (price, colour, vibe) since
the search API only supports exact category and substring-of-name matching.
Verified end-to-end against both real APIs (a "cheap kitchen item" request
correctly picked the cheapest real match with a real item ID). It can only
stage a suggestion via a `recommend_product` tool — the actual purchase still
goes through the existing, separately-verified `BuyButton`/`buyNow` flow
behind a manual click, so the assistant itself can never place a real order.
